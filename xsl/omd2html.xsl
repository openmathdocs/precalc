...
<xsl:stylesheet 
  ...
  <xsl:import href="./omd-common.xsl" />
...
<!-- Figure code -->
<xsl:template match="figure/code" xmlns:file="java.io.File">
  <!-- create $identifier variable to be figure-<chapter number>-<figure number> -->
      <xsl:variable name="identifier">
        ...
        <xsl:choose>
          <xsl:when test="ancestor::multobjects[@children='subfigure']">
...
<xsl:template match="table/caption">
  <xsl:element name="caption">
    ...
    <xsl:element name="a">
     ...
     <!-- subtable captions don't have 'table' in the caption -->
     <xsl:choose>
      <xsl:when test="not(../../../@children='subtable')">
        <!-- regular tables, e.g Table 1.3 -->
        <xsl:apply-templates select=".." mode="type-name"/>
        <xsl:text> </xsl:text>
        <xsl:apply-templates select=".." mode="number"/>
      </xsl:when>
      <xsl:otherwise>
        <!-- subtables, e.g (a), (b), (c), etc -->
...
<!-- tables receive their anchor and caption at the beginning -->
<xsl:template match="multobjects[@type='table']">
  <!-- create an anchor -->
  <xsl:if test="caption">
      <xsl:element name="a">
        <xsl:attribute name="class">anchor</xsl:attribute>
