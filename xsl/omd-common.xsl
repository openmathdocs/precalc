...
<xsl:strip-space elements="example remark exercise multicols hint solution" />
<xsl:strip-space elements="ul ol dl" />
<xsl:strip-space elements="md mdn" />
<xsl:strip-space elements="sage figure multobjects" />
...
<xsl:template match="table" mode="number">
  ...
  <xsl:apply-templates select="ancestor::chapter[1]" mode="number"/>
  <xsl:text>.</xsl:text>
  <xsl:number level="any" count="table[not(ancestor::multobjects[@children='subtable'])]|multobjects[@type='table' and @children='subtable']"/>
</xsl:template>
...
<xsl:template match="multobjects[@children='subtable']/div/table" mode="number">
  ...
  <xsl:text>.</xsl:text>
  <xsl:text>(</xsl:text>
  <xsl:number format="a" count="multobjects[@children='subtable']/div"/>
  <xsl:text>)</xsl:text>
</xsl:template>

<!-- subtables have captions (a), (b), so need a different mode -->
<xsl:template match="multobjects[@children='subtable']/div/table" mode="caption">
  <xsl:text>(</xsl:text>
  <xsl:number format="a" count="multobjects[@children='subtable']/div"/>
  <xsl:text>)</xsl:text>
</xsl:template>
